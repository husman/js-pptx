...
Presentation.prototype.load = function (data, done) {
  ...
  async.each(Object.keys(zip.files), function (key, callback) {
    ...
    if (ext == '.xml' || ext == '.rels') {
      ...
      content[key] = zip.file(key).asUint8Array();
      callback(null);
    }
  }, done);
};
...
Presentation.prototype.getSlideSize = function () {
  var slideSize = this.content["ppt/presentation.xml"]["p:presentation"]["p:sldSz"][0]['$'];

  return {
    width: parseInt(slideSize.cx),
    height: parseInt(slideSize.cy),
  };
};

Presentation.prototype.getSlideMaster = function (name) {
  return new Slide({content: this.content['ppt/slideMasters/' + name + '.xml'], presentation: this, name: name});
};
