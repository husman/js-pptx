...
Slide.prototype.getPics = function () {
  var shapes = this.content["p:sld"]["p:cSld"][0]["p:spTree"][0];
  var images = [];

  if (!shapes || !shapes['p:pic']) {
    return;
  }

  images = shapes['p:pic'].map(pic => new Picture(pic, this.presentation, this.name));
